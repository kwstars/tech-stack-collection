...
func main() {
	...
	go func() {
		...
		execSQL(sessionA, "A", "SELECT * FROM t WHERE id > 10 and id <= 15 FOR UPDATE;")
		t2 <- true
	...
	go func() {
		...
		<-t2
		t3 <- true
		execSQL(sessionB, "B", "UPDATE t SET d = d + 1 WHERE id = 20;")
		// execSQL(sessionB, "B", "UPDATE t SET d = d + 1 WHERE id = 15;")
	...
	go func() {
		...
		execSQL(sessionC, "C", "INSERT INTO t VALUES(16, 16, 16);")
