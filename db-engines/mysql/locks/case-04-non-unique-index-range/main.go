...
func main() {
	...
	go func() {
		...
		execSQL(sessionA, "A", "SELECT * FROM t WHERE c >= 10 AND c < 11 FOR UPDATE;")
		t2 <- true
	...
	go func() {
		...
		<-t2
		t3 <- true
		execSQL(sessionB, "B", "INSERT INTO t VALUES(16, 16, 16);")
		execSQL(sessionB, "B blocked", "INSERT INTO t VALUES(8, 8, 8);")
	...
	go func() {
		...
		execSQL(sessionC, "C", "UPDATE t SET d = d + 1 WHERE c = 5;")
		execSQL(sessionC, "C blocked", "UPDATE t SET d = d + 1 WHERE c = 15;")
