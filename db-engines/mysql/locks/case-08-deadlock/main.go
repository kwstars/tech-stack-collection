...
func main() {
	...
	go func() {
		...
		execSQL(sessionA, "A", "SELECT id FROM t WHERE c = 10 LOCK IN SHARE MODE;")
		t2 <- true

		time.Sleep(1 * time.Second)
		execSQL(sessionA, "A", "INSERT INTO t VALUES(8, 8, 8);")
	...
	go func() {
		...
		<-t2
		execSQL(sessionB, "B blocked", "UPDATE t SET d = d + 1 WHERE c = 10;")
