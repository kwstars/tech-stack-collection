...
Lists 是简单的字符串列表，按照插入顺序排序。你可以添加一个元素到列表的头部（左边）或者尾部（右边）。
...
- Sentinel 是独立运行的程序，它可以与 Redis 服务器分开部署。你可以使用类似以下的命令来启动 Sentinel：`redis-server /path/to/sentinel.conf --sentinel`。
