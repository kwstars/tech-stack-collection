...
func main() {
	...
	scheme1.AddKnownTypes(schema.GroupVersion{Group: "", Version: "v1"}, &corev1.Pod{}, &corev1.ConfigMap{})

	scheme2 := runtime.NewScheme()
	scheme2.AddKnownTypes(schema.GroupVersion{Group: "apps", Version: "v1"}, &appsv1.Deployment{})
	scheme2.AddKnownTypes(schema.GroupVersion{Group: "apps", Version: "v1beta1"}, &appsv1beta1.Deployment{})

	// ## Mapping
	types := scheme2.KnownTypes(schema.GroupVersion{Group: "apps", Version: "v1"})
	fmt.Printf("known types for scheme2: %v\n", types)
	// map[Deployment:v1.Deployment]

	groupVersions := scheme2.VersionsForGroupKind(schema.GroupKind{Group: "apps", Kind: "Deployment"})
	...
	scheme2.AddConversionFunc((*appsv1.Deployment)(nil), (*appsv1beta1.Deployment)(nil),
	...
	v1deployment.APIVersion, v1deployment.Kind = appsv1.SchemeGroupVersion.WithKind("Deployment").ToAPIVersionAndKind()
	...
	jsonSerializer := jsonserializer.NewSerializerWithOptions(jsonserializer.SimpleMetaFactory{}, scheme2, scheme2,
	...
	mapping, _ := restMapper.RESTMapping(schema.GroupKind{Group: "apps", Kind: "Deployment"})
	fmt.Printf("single mapping: %+v\n", *mapping) // single mapping: {Resource:apps/v1, Resource=deployments GroupVersionKind:apps/v1, Kind=Deployment Scope:<nil>}

	mappings, _ := restMapper.RESTMappings(schema.GroupKind{Group: "apps", Kind: "Deployment"})
	for _, mapping := range mappings {
		fmt.Printf("mapping: %+v\n", *mapping)
	}
	// mapping: {Resource:apps/v1, Resource=deployments GroupVersionKind:apps/v1, Kind=Deployment Scope:<nil>}
	// mapping: {Resource:apps/v1beta1, Resource=deployments GroupVersionKind:apps/v1beta1, Kind=Deployment Scope:<nil>}

	kinds, _ := restMapper.KindsFor(
		schema.GroupVersionResource{Group: "", Version: "", Resource: "deployment"},
	)
	fmt.Printf("kinds: %+v\n", kinds) // [apps/v1, Kind=Deployment apps/v1beta1, Kind=Deployment]

	resources, _ := restMapper.ResourcesFor(
		schema.GroupVersionResource{Group: "", Version: "", Resource: "deployment"},
	)
	fmt.Printf("resources: %+v\n", resources) // resources: [apps/v1, Resource=deployments apps/v1beta1, Resource=deployments]
}
