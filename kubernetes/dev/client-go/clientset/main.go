...
func main() {
	...
	createdPod, err := clientset.CoreV1().Pods("project1").Create(ctx, &wantedPod, metav1.CreateOptions{})
	if err != nil {
	...
	pod, err := clientset.CoreV1().Pods("project1").Get(ctx, "nginx-pod", metav1.GetOptions{})
	if err != nil {
		if errors.IsNotFound(err) {
	...
	podListInNS, err := clientset.CoreV1().Pods("project1").List(ctx, metav1.ListOptions{})
	if err != nil {
		panic(err)
	}
	...
	podListInCluster, err := clientset.CoreV1().Pods("").List(ctx, metav1.ListOptions{})
	if err != nil {
		panic(err)
	}
	...
	// ### Using Requirements
	req1, err := labels.NewRequirement("mykey", selection.Equals, []string{"value1"})
	if err != nil {
		panic(err)
	}
	...
	filteredPodListInCluster, err := clientset.CoreV1().Pods("").List(ctx, metav1.ListOptions{LabelSelector: s})
	if err != nil {
		panic(err)
	}
	...
	ssaDep.APIVersion, ssaDep.Kind = appsv1.SchemeGroupVersion.
		WithKind("Deployment").
		ToAPIVersionAndKind()
