...
func NewController(podListWatcher cache.ListerWatcher, queue workqueue.RateLimitingInterface) *Controller {
...
func (c *Controller) syncToStdout(key string) error {
	...
	if !exists {
		...
		klog.Infof("Pod %s does not exist anymore, it might have been deleted", key)
	} else {
		klog.Infof("Sync/Add/Update for Pod %s", obj.(*corev1.Pod).GetName())
	}
	return nil
}
...
func main() {
	...
	controller := NewController(podListWatcher, queue)
