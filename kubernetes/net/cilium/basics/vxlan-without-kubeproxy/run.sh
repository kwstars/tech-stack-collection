...
readonly KIND_NAME="vxlan-without-kubeproxy"
...
# Capture packet
docker exec -d vxlan-without-kubeproxy-control-plane bash -c 'tcpdump -pen -i eth0 udp -w /data/control-plane-eth0.pcap'
docker exec -d vxlan-without-kubeproxy-worker bash -c 'tcpdump -pen -i eth0 udp -w /data/worker1-eth0.pcap'
docker exec -d vxlan-without-kubeproxy-worker2 bash -c 'tcpdump -pen -i eth0 udp -w /data/worker2-eth0.pcap'
