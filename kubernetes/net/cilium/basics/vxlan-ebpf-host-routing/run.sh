...
readonly KIND_NAME="vxlan-ebpf-host-routing"
...
# Capture packet
docker exec -d ${KIND_NAME}-control-plane bash -c 'tcpdump -pen -i eth0 udp -w /data/control-plane-eth0.pcap'
docker exec -d ${KIND_NAME}-worker bash -c 'tcpdump -pen -i eth0 udp -w /data/worker1-eth0.pcap'
docker exec -d ${KIND_NAME}-worker2 bash -c 'tcpdump -pen -i eth0 udp -w /data/worker2-eth0.pcap'
