...
readonly KIND_NAME="native-routing-with-kubeproxy"
...
# Capture packet
docker exec -d native-routing-with-kubeproxy-control-plane bash -c 'tcpdump -pen -i eth0 udp -w /data/control-plane-eth0.pcap'
docker exec -d native-routing-with-kubeproxy-worker bash -c 'tcpdump -pen -i eth0 udp -w /data/worker1-eth0.pcap'
docker exec -d native-routing-with-kubeproxy-worker2 bash -c 'tcpdump -pen -i eth0 udp -w /data/worker2-eth0.pcap'
