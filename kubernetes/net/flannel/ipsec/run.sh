...
# 1.prep noCNI env
kind create cluster --name=flannel-ipsec --image=mykindest/node:v1.28.7 --config=./kind.yaml
...
# Captrue packet
docker exec -d flannel-ipsec-control-plane bash -c "tcpdump -pen -i eth0 -w /data/control-eth0.pcap"
