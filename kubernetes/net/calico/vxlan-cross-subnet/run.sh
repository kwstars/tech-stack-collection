...
# 1.prep noCNI env
kind create cluster --name=calico-vxlan-cross-subnet --image=mykindest/node:v1.28.7 --config=./kind.yaml
...
# Captrue packets
docker exec -d calico-vxlan-cross-subnet-control-plane bash -c "tcpdump -pen -i net0 -w /data/control-net0.pcap"
docker exec -d clab-calico-vxlan-cross-subnet-gw0 bash -c "tcpdump -pen -i eth1 -w /data/gw0-eth1.pcap"
